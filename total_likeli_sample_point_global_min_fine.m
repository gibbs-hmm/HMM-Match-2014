...
function total_likeli_prob = total_likeli_sample_point_global_min_fine(sample_seq,pi_0,log_pi_begin,log_tao_x_begin,log_tao_y_begin,log_tao_x_end,log_tao_y_end,log_delta_r,ratio_track,dis_bin_input_new,mu,sigma,delta_grid,input_scaled_new,mean_tar,transition_prob_track,ratio_track_new,group_transition)
...
%     end
    %pre_index = ratio_track(dis_bin_input(i),ali_seq(i)-ali_seq(i-1)+1);
...
%         
        ...
        %         tmp = ali_seq(i)-ali_seq(i-1)+1;
        %         if dis_bin_input(i)~=0 && tmp~=0
        %             curr_index = ratio_track(dis_bin_input(i),tmp);
        %         elseif tmp~=0
        %             curr_index=ratio_track_0(tmp);
        %         elseif dis_bin_input(i)~=0
        %             curr_index=ratio_track_1(dis_bin_input(i));
        %         else
        %             curr_index=9;
        %
        %         end
        %curr_index = ratio_track(dis_bin_input(i),ali_seq(i)-ali_seq(i-1)+1);
       ...
       % density_rr = density_mixture_gaussian(log(curr_rr),mix_mu1,mix_std1,mix_mu2,mix_std2,mix_p1,mix_p2);
        ...
        log_prob = log_prob + log(group_transition(pre_qq,curr_qq)*density_rr)+log_grid(i,ali_seq(i))+log_delta_r(1);
        pre_rr = curr_rr;
