...
log_f = zeros(length(input_scaled_new),length(discrete_target), 4*max(dis_bin_input_new));
...
for i=1:length(input_scaled_new)-1
    if i<=length(input_scaled_new)-gap_con
        ...
        for k=1:length(discrete_target)
            if length(discrete_target)-k+1>0 && length(discrete_target)-k+1<=size(log_f,3)
            if log_f(i,length(discrete_target),length(discrete_target)-k+1)==-inf
                gap_add_f_x(i,k)=-inf;
            else
                gap_add_f_x(i,k) = log_f(i,length(discrete_target),length(discrete_target)-k+1)+log_delta_r(2)+log_tao_x_end(1)*...
...
for j=1:length(discrete_target)-1
    if j<length(discrete_target)-gap_con_tar
        ...
        for k=1:length(discrete_target)-1
            if j-k+1>size(log_f,3) || j-k+1<1
                gap_add_f_y(j,k)=-inf;
            elseif log_f(length(input_scaled_new),j,j-k+1)==-inf
                gap_add_f_y(j,k)=-inf;
            else
                gap_add_f_y(j,k) = log_f(length(input_scaled_new),j,j-k+1)+log_delta_r(3)+log_tao_y_end(1)*...
...
for i=1:length(discrete_target)
    if length(discrete_target)-i+1>0 && length(discrete_target)-i+1<=size(log_f,3)
    gap_add_f_e(i) = log_f(length(input_scaled_new),length(discrete_target),length(discrete_target)-i+1)+log_delta_r(3);
    else
        gap_add_f_e(i)=-inf;
    end
end
